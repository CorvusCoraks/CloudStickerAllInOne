...
import ru.gremal.cloudsticker.common.ui.AbstractUIDetonator;
import ru.gremal.cloudsticker.io.IO;
//import ru.gremal.cloudsticker.common.ui.UIDetonator;
...
public class GUI extends AbstractUI {
    ...
    private IOSpecific io;
    // становится true, если нечто (в т. ч. пользователь) инициирует закрытие приложения
    // Это поле - сигнал для отдельных работающих потоков UI
    //protected static boolean closeUIEvent = false;
    ...
    * */
    public class MainWindow extends JFrame
    {
        ...
        // Команда на завершение всех нитей, порождённых событиями изменений в поле заметки и в полях меток устройств
        protected boolean resumeAllChangeThreads = false;
...
*/
        ...
        class MyWindowListener extends WindowAdapter {
            ...
            */
            public void windowClosing(WindowEvent Ev) {
                ...
                // блокируем окно
                setUIDisable();
                // Команда на завершение нитей
                resumeAllChangeThreads = true;
                ...
                */
                ...
                try {
                    // Спим, пока не завершатся все нити, сигнализирующие о том, что были внесены изменения.
                    // Т. е. ждём их спокойной замостоятельной смерти
                    while(SwingUI.getTotalChangeEventCounter() !=0){
                        Thread.sleep(FIELD_CHANGE_TIMEOUT);
                    }
        ...
        class SynchronisationButtonListener implements ActionListener
        {
            ...
            class newThread extends Thread
            {
                public void run(){
                    ...
                    */
                    // Разрешаем нити событий изменений в полях снова
                    resumeAllChangeThreads = false;
                    setUIEnable();
                }
            }
        }
        ...
        class InviteOrKickButtonListener implements ActionListener
        {
            ...
            class newThread extends Thread
            {
                ...
                public void run() {
                    ...
                    // todo Необходима проверка на длину введённого пароля (если он введён). Если что - обрезаем
        ...
        class DeviceLabelTextFieldListener implements CaretListener
        {
            ...
            class newThread extends Thread
            {
                ...
                public void run(){
                    ...
                    new Thread(){
                        ...
                        public void run() {
                            try{
                                ...
                                SwingUI.incTotalChangeEventCounter();
                                // ждём отведённое время, проверяясь на команду закрытия приложения
                                int i = 100;
                                while (i > 0) {
                                    sleep(FIELD_CHANGE_TIMEOUT/100);
                                    // команда на завершение нити
                                    if(resumeAllChangeThreads){break;}
                                    i--;
                                }
                                // уменьшаем счётчик процессов
                                // int counter = stf.decChangeEventCounter();
                                SwingUI.decTotalChangeEventCounter();
                    ...
                    //}
                    /*
                    // todo Необходима проверка на длину метки. Если что - обрезаем
                    ...
                    new Thread(){
                        ...
                        public void run() {
                            try{
                                ...
                                SwingUI.incTotalChangeEventCounter();
                                // тест
                                //System.out.println(frame.noteArea.getChangeEventCounter());
                                int i = 100;
                                while (i > 0) {
                                    sleep(FIELD_CHANGE_TIMEOUT/100);
                                    // команда на завершение нити
    ...
    /* Обязательный класс и его реализация для UI, который запускается из ядра
    * Вызов: GUI.UIDetonator.createUiInstance(CoreSide core); */
    public static class UIDetonator extends AbstractUIDetonator {
        // Данная функция, собственно, и создаёт экземпляр UI
        public static UISide createUiInstance(CoreSide core){
            //GUI gui;
            UIDetonator.core = core;
            SwingUtilities.invokeLater(new GUIRunnable());
            // спим, пока не будет создан экземпляр UI
            try {
                while(UIDetonator.ui == null){ Thread.sleep(50); }
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            return UIDetonator.ui;
        };

        protected static class GUIRunnable implements Runnable{
            public void run(){
                try {
                    UIDetonator.ui = new GUI(UIDetonator.core);
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
                // todo сделать выдачу предупреждающего окна
                if (UIDetonator.ui == null) { /* todo сделать выдачу предупреждающего окна */
                    return;
                }
            }
        }
    ...
    public GUI(CoreSide core) throws ClassNotFoundException
    {
        this.core = core;
        this.io = new IO();
        ...
        this.setMaxCompanyCount(core.getMaxCompanyCount());
        this.setMaxCharsInNote(core.getMaxCharsInNote());
        this.setMaxCharsInLabel(core.getMaxCharsInLabel());
        this.setMaxCharsInInvitationPass(core.getMaxCharsInInvitationPass());

        this.core.getLogFileService().publishToLogFile("Конструктор GUI");
        /*
        try {
            //CommonTools.publishToLogFile(this.core, "Конструктор GUI");
            this.core.getLogFileService().publishToLogFile("Конструктор GUI");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
*/
        ...
        localisation();
        this.core.getLogFileService().publishToLogFile("Локализация прошла");
/*
        try {
            this.core.getLogFileService().publishToLogFile("Локализация прошла");
        } catch (ClassNotFoundException e) {
        ...
        */
        ...
        this.core.getLogFileService().publishToLogFile("frame:" + frame.toString());
/*
        try {
            this.core.getLogFileService().publishToLogFile("frame:" + frame.toString());
        } catch (ClassNotFoundException e) {
...
*/
    ...
    private void localisation(){
        try{
            localisation = CommonTools.readFromIniFile(io, locFileName);
...
/*    Чистка свободного текстового поля в GUI-массиве устройств круга и, если на парной кнопке осталась старая
    ...
    @Override
    public synchronized void setUIEnable(){
    ...
    @Override
    public synchronized void setUIDisable(){
        if(!frame.synchronisationButton.isEnabled()){ return; } // если уже другой нитью всё установлено. Не фига по второму разу
